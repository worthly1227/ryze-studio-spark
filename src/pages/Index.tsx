...
const IMessageIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
    <path d="M12 2C6.477 2 2 5.813 2 10.5c0 2.614 1.386 4.946 3.555 6.523-.138.912-.552 2.396-1.555 3.477 0 0 2.37-.158 4.282-1.547A11.26 11.26 0 0012 19c5.523 0 10-3.813 10-8.5S17.523 2 12 2z"/>
  </svg>
);
...
const ProofCard: React.FC<{ item: typeof proofItems[0] }> = ({ item }) => {
  ...
  const shareOptions = [
    ...
    },
    {
      label: "WhatsApp",
      icon: <WhatsAppIcon />,
      onClick: () => { window.open(`https://wa.me/?text=${encodeURIComponent(shareText)}`, "_blank"); setShowShareModal(false); },
    },
    {
      label: "iMessage",
      icon: <IMessageIcon />,
      onClick: () => { window.open(`sms:&body=${encodeURIComponent(shareText)}`); setShowShareModal(false); },
    },
    {
      label: "Facebook",
      icon: <Facebook className="w-6 h-6" />,
      onClick: () => { window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(siteUrl)}&quote=${encodeURIComponent(shareText)}`, "_blank"); setShowShareModal(false); },
    },
    {
      label: "Email",
      icon: <Mail className="w-6 h-6" />,
      onClick: () => { window.open(`mailto:?subject=${encodeURIComponent("Check out Ryze Studios")}&body=${encodeURIComponent(shareText)}`); setShowShareModal(false); },
    },
  ];
      ...
      <AnimatePresence mode="wait">
        ...
        {(phase === 1 || phase === 2 || phase === 3) && (
          ...
          >
            ...
            <div className="flex items-baseline font-heading font-bold text-black text-xl sm:text-3xl md:text-5xl whitespace-nowrap">
              <AnimatePresence>
                {phase === 1 && (
                  <motion.span
                    key="meet"
