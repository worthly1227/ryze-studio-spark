...
import { Search, Play, Download, Loader2, Film, Clock, User } from "lucide-react";
...
const MyVideos: React.FC = () => {
  ...
  const handleDownload = async (video: PexelsVideo) => {
    ...
    toast({ title: "Download started", description: `"${query}" clip by ${video.user.name}` });
  };
        ...
        <div className="space-y-4">
            ...
            {results.total_results > results.per_page && (
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() => searchVideos(results.page - 1)}
                >
                  Previous
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                  disabled={results.page * results.per_page >= results.total_results || loading}
                  onClick={() => searchVideos(results.page + 1)}
                >
                  Next
                </Button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {results.videos.map((video) => (
              <Card
                key={video.id}
                className="group overflow-hidden cursor-pointer border-border hover:border-primary/40 transition-colors"
            ...
            <>
              <DialogHeader className="p-4 pb-0">
                <DialogTitle className="flex items-center gap-2">
                  ...
                  Video by {previewVideo.user.name}
                </DialogTitle>
              </DialogHeader>
              <div className="aspect-video bg-black">
                <video
                  src={getBestFile(previewVideo.video_files)?.link}
                  controls
                  autoPlay
              ...
              <div className="p-4 flex items-center justify-between">
                ...
                <Button onClick={() => handleDownload(previewVideo)}>
